...
function renderPluginModal(pluginId: string, definition: PluginModalDefinition): void {
    ...
    const title = modal.querySelector<HTMLElement>(`#${CSS.escape(modal.id)}-title`);
    ...
    const makeAlign = (align?: PluginModalAlign): string => {
        if (align === 'centered') return 'center';
        if (align === 'right') return 'flex-end';
        return 'flex-start';
    };

    const addButtonRow = (button: HTMLButtonElement, align?: PluginModalAlign): void => {
        const row = document.createElement('div');
        row.style.display = 'flex';
        row.style.flexWrap = 'wrap';
        row.style.gap = '.5rem';
        row.style.justifyContent = makeAlign(align);
    ...
    for (const item of Array.isArray(definition.content) ? definition.content : []) {
        ...
        if (item.type === 'list') {
            ...
            for (const row of items) {
                ...
                for (const action of Array.isArray(row.actions) ? row.actions : []) {
                    ...
                    button.dataset.pluginAction = String(action.id || '').trim();
                    button.dataset.pluginId = pluginId;
                    button.dataset.pluginModal = modal.id;
                    if (action.payload) {
                        button.dataset.pluginPayload = JSON.stringify(action.payload);
                    }
        ...
        if (item.type === 'button' || typeof (item as PluginModalButtonDefinition).content === 'string') {
            const buttonDef = item as PluginModalButtonDefinition;
            const button = document.createElement('button');
            button.type = 'button';
            button.className = 'tbtn';
            ...
            addButtonRow(button, buttonDef.align);
        }
    }
