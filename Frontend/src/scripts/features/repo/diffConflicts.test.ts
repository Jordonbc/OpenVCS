...
vi.mock('../../state/state', () => ({ state: mockState }));

const mockDiffEl = document.createElement('div');
vi.mock('./context', () => ({ diffEl: mockDiffEl }));
vi.mock('./hydrate', () => ({ hydrateStatus: vi.fn() }));
vi.mock('./diffBinary', () => ({ scrollDiffToTop: vi.fn() }));
vi.mock('../conflicts', () => ({
...
describe('renderConflictMarkup (via full render)', () => {
  ...
  it('escapes path using escapeHtml', async () => {
    const { escapeHtml } = await import('../../lib/dom');
    mockInvoke.mockResolvedValue({
