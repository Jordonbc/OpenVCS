...
vi.mock('@scripts/features/repo/list', () => ({ renderList: vi.fn() }));
vi.mock('@scripts/features/conflicts', () => ({ autoOpenFirstConflict: vi.fn() }));
...
describe('hydrateStatus selection reconciliation', () => {
  it('uses one backend snapshot for all repo hydration calls', async () => {
    ...
    const { hydrateBranches, hydrateStatus, hydrateCommits, hydrateStash, hydrateVcsActionLabels } = await import('@scripts/features/repo/hydrate');
...
describe('hydrateVcsActionLabels', () => {
  it('resolves action labels from backend', async () => {
    ...
    const { hydrateVcsActionLabels } = await import('@scripts/features/repo/hydrate');
