...
vi.mock('@scripts/ui/modals', () => ({ closeModal: vi.fn() }));
vi.mock('@scripts/plugins', () => ({ runHook: vi.fn(async () => ({ cancelled: false })) }));
vi.mock('@scripts/state/state', () => ({ state: { branch: 'main', branches: [] } }));
