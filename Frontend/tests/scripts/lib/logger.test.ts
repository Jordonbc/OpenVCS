...
describe('logger', () => {
  ...
  it('prefixes module logs and serializes plain objects', async () => {
    const { logger } = await import('@scripts/lib/logger');
    const { TAURI } = await import('@scripts/lib/tauri');
    const { addFrontendLogBreadcrumb } = await import('@scripts/lib/monitoring');
...
it('formats errors with their stack and routes them as error logs', async () => {
  ...
  it('skips breadcrumb when options.breadcrumb is explicitly false', async () => {
    const { __testOnlySendToBackend } = await import('@scripts/lib/logger');
    const { addFrontendLogBreadcrumb } = await import('@scripts/lib/monitoring');
  ...
  it('patches console methods and forwards log payloads only once', async () => {
    ...
    await import('@scripts/lib/logger');
    const { TAURI } = await import('@scripts/lib/tauri');
    const { addFrontendLogBreadcrumb } = await import('@scripts/lib/monitoring');
