...
use super::shared::open_repo_and_store;
...
pub async fn set_vcs_backend_cmd(
    ...
    if let Some(repo) = state.current_repo() {
        ...
        open_repo_and_store(&state, &path, backend_id, "set_vcs_backend_cmd", |error| {
            error!(
            ...
            format!("Failed to reopen repo with `{backend_label}`: {error}")
        })
        .await?;
...
pub async fn reopen_current_repo_cmd(state: State<'_, AppState>) -> Result<(), String> {
    ...
    open_repo_and_store(
        ...
        &path,
        backend_id,
        "reopen_current_repo_cmd",
        |error| format!("Failed to reopen repo with `{backend_label}`: {error}"),
    )
    .await?;
    Ok(())
}
