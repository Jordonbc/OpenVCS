...
fn serializes_settings_enums_with_kebab_case_names() {
    assert_eq!(
        serde_json::to_value(Theme::Dark).expect("theme"),
        serde_json::json!("dark")
    );
    assert_eq!(
        serde_json::to_value(Language::EN).expect("language"),
        serde_json::json!("e-n")
    );
    assert_eq!(
        serde_json::to_value(UpdateChannel::Nightly).expect("channel"),
        serde_json::json!("nightly")
    );
    assert_eq!(
        serde_json::to_value(SshBinary::Custom).expect("ssh binary"),
        serde_json::json!("custom")
    );
    assert_eq!(
        serde_json::to_value(HookPolicy::Allow).expect("hook policy"),
        serde_json::json!("allow")
    );
    assert_eq!(
        serde_json::to_value(CredentialHelper::None).expect("helper"),
        serde_json::json!("none")
    );
    assert_eq!(
        serde_json::to_value(SshAgent::OnePassword).expect("ssh agent"),
        serde_json::json!("one-password")
    );
    assert_eq!(
        serde_json::to_value(WhitespaceMode::Eol).expect("whitespace"),
        serde_json::json!("eol")
    );
    assert_eq!(
        serde_json::to_value(EditorChoice::Clion).expect("editor"),
        serde_json::json!("clion")
    );
    assert_eq!(
        serde_json::to_value(IssueProvider::Forgejo).expect("provider"),
        serde_json::json!("forgejo")
    );
    assert_eq!(
        serde_json::to_value(ColorBlindMode::Deuteranopia).expect("color blind"),
        serde_json::json!("deuteranopia")
    );
    assert_eq!(
        serde_json::to_value(ForcePushPolicy::TrackedRemotes).expect("force push"),
        serde_json::json!("tracked-remotes")
    );
    assert_eq!(
        serde_json::to_value(ProxyMode::Manual).expect("proxy mode"),
...
fn section_defaults_stay_aligned_with_schema() {
    ...
    assert_eq!(vcs.ssh_binary, SshBinary::Auto);
    ...
    assert_eq!(
        templates.commit_message_template_create,
        "Create {file:name}"
    );
    assert_eq!(
        templates.commit_message_template_update,
        "Update {file:name}"
    );
    assert_eq!(
        templates.commit_message_template_delete,
        "Delete {file:name}"
    );
...
"#,
    ...
    assert_eq!(
        cfg.commit.commit_templates.commit_message_template_delete,
        "Delete {file:name}"
    );
    assert_eq!(cfg.credentials.helper, CredentialHelper::OsKeychain);
...
fn round_trips_configuration_through_toml() {
    ...
    cfg.integrations
        .host_overrides
        .insert("example.com".into(), IssueProvider::Forgejo);
