...
flatpak install -y flathub org.freedesktop.Sdk.Extension.node24//49
...
- It uses `org.freedesktop.Sdk.Extension.rust-stable` for `cargo`/`rustc` and `org.freedesktop.Sdk.Extension.node24` for `npm` inside the Flatpak build environment.
